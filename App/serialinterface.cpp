...
QStringList SerialInterface::portNames() {
  QStringList names;
...
QString SerialInterface::port() const { return m_port; }

int SerialInterface::baud() const { return m_serial.baudRate(); }
...
void SerialInterface::setPort(QString port) {
  if (m_port == port)
    return;
